...
function SelectDate(objId,strFormat,x,y)
{
 ...
 if(document.getElementById("ContainerPanel")==null){InitContainerPanel();}
    var date = new Date();
    var by = date.getFullYear()-100;  //最小值 → 100 年前
...
String.prototype.toDate = function(style) {
  ...
  if(d == null ||d == "" || isNaN(d)) {oldd = d;d = new Date().getDate();}
  if(M == null ||M == "" || isNaN(M)) 
  {
  ...
  {
  	if(oldd == null ||oldd == "" || isNaN(oldd))
  	{
  		var lastday = (new Date()).getLastDays(parseInt(M)-1);
  		if(d>lastday)
  		{
  			d=lastday;
  		}
...
Date.prototype.getLastDays=  function(month)
{
	var ar = new Array(13);
...
//2009-03-03 添加的代码，放置时间的行
  ...
  {
  	calendar.day = (new Date()).getLastDays(calendar.month);
  }
...
Calendar.prototype.show = function (dateObj, popControl) {
 ...
 this.bindSecond();
  ...
  this.panel.style.top = (xy.y + topY + dateObj.offsetHeight - getscollposition(popControl)) + "px";
