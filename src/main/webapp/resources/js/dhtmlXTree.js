 ...
 for(var i=1;i<=itemObject.childsCount;i++)
{
 ...
 that.TreeRowClick(this.parentObject.id,that._getOpenState(this.parentObject));
 ...
 if(that.autoopen){
 	 //if(!this.saveOpenStates());
 	 //that.saveOpenStates();
 	 that.SetOpenCookies(that.rootId);
 	}
};
 ...
 if(targetObject.childsCount>=2)
{
 ...
 //this.TreeRowClick(item.id,this._getOpenState(item));
...
/*dhtmlXTreeObject.prototype.AddTreeList=function(item){
	color1='#ff0000';color2='#ff0000';
	var insertNum = 1;
	var insertRes = -1;
	while(insertNum > 0){//?????????????????
		insertNum = 0;
		for(i=0;i+3<item.length;i=i+4){//??????
			if(item[i] == "") continue;//??????
			insertRes = this.insertNewItem(item[i],item[i+1],item[i+2],0,"folderClosed.gif","folderOpen.gif","folderClosed.gif","");
			
			if(insertRes != -1 ) {//????
				if(item[i+3] == 0) tree.setItemColor(item[i+1],color1,color2);
	...
	for(i=0;i+3<item.length;i=i+4){//???????
		...
		if(insertRes == -1 ) {//???????????
			this.insertNewItem("0",item[i+1],item[i+2],0,"folderClosed.gif","folderOpen.gif","folderClosed.gif","");
			
		}
		item[i] = "";
		if(item[i+3] == 0) this.setItemColor(item[i+1],color1,color2);
	}
	this.autoopen=true;
	this.rootId='0';
	if(!this.GetOpenCookies()) this.closeAllItems(this.rootId); 
...
var treeItemList;
var treeCheckList;
var treeHideItemList;
var treeShowItemList;
var treeHideChildList;
var treeDisableCheckList;
var isJustNodeCheck;
var loadMode=0;//0:一次加载，1:多次加载


dhtmlXTreeObject.prototype.AddTreeList=function(item){
	treeItemList = item;
	this.InitTreeByParentId("0",'1');
	this.closeAllItems(this.rootId);
	var self=this;
	setTimeout(function(){self.GetOpenCookies();},0);
}
dhtmlXTreeObject.prototype.InitTreeByParentId=function(parentId,loadtype){
	if(loadMode == 0 && loadtype=='2'){
		return;
	}
	var i=0;
	color1='#ff0000';color2='#ff0000';
	...
	for(i=0;i<treeItemList.length;i++){
		if( typeof(treeItemList[i]) == "undefined" ){
			continue;
		}
		if(treeItemList[i] == "") continue;//??????
		if(treeItemList[i][0] == "") continue;//??????
		
		if(loadMode == 1){
			if(treeItemList[i][0] != parentId) continue;			
		}
		pic_simple = "folderClosed.gif"; pic_open = "folderOpen.gif"; pic_close = "folderClosed.gif";
		if(treeItemList[i].length > 6){
			pic_simple = treeItemList[i][4];
			pic_open = treeItemList[i][5];
			pic_close = treeItemList[i][6];
		}
		insertRes = this.insertNewItem(treeItemList[i][0],treeItemList[i][1],treeItemList[i][2],0,pic_simple,pic_open,pic_close,"");
		if(insertRes == -1){
			insertRes = this.insertNewItem('0',treeItemList[i][1],treeItemList[i][2],0,pic_simple,pic_open,pic_close,"");
		}		
		if(insertRes != -1 ) {//????
			if(treeItemList[i][3] == 0) this.setItemColor(treeItemList[i][1],color1,color2);
			if(loadMode == 1){
				var firstChild = this.GetFirstChildId(treeItemList[i][1]);
				if(firstChild != ""){
					var ll = firstChild.split("\t");
					this.insertNewItem(ll[0],ll[1],ll[2],0,pic_simple,pic_open,pic_close,"");
				}				
			}
		}
	}
	this.autoopen=true;
	this.rootId='0';
	this.closeAllItems(parentId);
	this.CheckList(treeCheckList);
	this.HideChildList(treeHideChildList);
	this.DisableCheckList(treeDisableCheckList);
	this.HideItemList(treeHideItemList);
	this.ShowItemList(treeShowItemList);
}
dhtmlXTreeObject.prototype.GetFirstChildId=function(parentId){
	/*for(var i=0;i+3<treeItemList.length;i=i+4){//??????
		if(treeItemList[i] == "") continue;//??????
		if(treeItemList[i] == parentId) {
			return treeItemList[i]+"\t"+treeItemList[i+1]+"\t"+treeItemList[i+2];
		}
	}*/
	for(var i=0;i<treeItemList.length;i++){//??????
		if(treeItemList[i] == "") continue;//??????
		if(treeItemList[i][0] == "") continue;//??????
		if(treeItemList[i][0] == parentId) {
			return treeItemList[i][0]+"\t"+treeItemList[i][1]+"\t"+treeItemList[i][2];
		}
	}
	return "";
}
dhtmlXTreeObject.prototype.TreeRowClick=function(nodeid, mode){
	if(loadMode == 0){
		return;
	}
	if(mode > 0){
		this.deleteChildItems(nodeid);
		var firstChild = this.GetFirstChildId(nodeid);
		if(firstChild != ""){
			var ll = firstChild.split("\t");
			this.insertNewItem(ll[0],ll[1],ll[2],0,"folderClosed.gif","folderOpen.gif","folderClosed.gif","");
		}
	}
	else if(mode < 0){
		this.InitTreeByParentId(nodeid,'2');
	}
}

dhtmlXTreeObject.prototype.SetSavStr=function(str){
	//this.savString=str;
	this.savString=str;
}
dhtmlXTreeObject.prototype.SetOpenCookies=function(nodeid){
	var i=0;	
	var nodStr = this.getAllSubItems(nodeid);
	var item = nodStr.split(',');
	//var item = this.getAllSubItems(nodeid);
	var cookieStr='';
	for(i=0;i<item.length;i++){
		if(this.getOpenState(item[i]) == 1){
			cookieStr += item[i]+',';
		}
	}
	//setCookie('mytreeopenids',cookieStr);
	this.savString[0] = cookieStr;
	//alert("set"+this.savString);
}

dhtmlXTreeObject.prototype.GetOpenCookies=function(){
	//var cookieStr = getCookie('mytreeopenids');
	var cookieStr = this.savString[0];
	this.closeAllItems(this.rootId);
	if(cookieStr == null || cookieStr == "undefined" || cookieStr == "")
		return false;
	var item = cookieStr.split(',');
	var i=0;
	var dealNum = 1;
	var dealRes = -1;
	while(dealNum > 0){
		...
		for(i=0;i<item.length;i++){
			if(item[i] == "") continue;
			var parentId = this.getParentId(item[i]);
			if(this.getOpenState(parentId) == 1){
				this.InitTreeByParentId(item[i],'2');
				this.openItem(item[i]);
				dealNum = dealNum+1;
				item[i] = "";
			}
		}
...
dhtmlXTreeObject.prototype.CheckList=function(item){
	if(item == null) return;
	treeCheckList = item;
	var i=0;
	for(i=0;i<item.length;i++){
		this.setCheck(item[i],true);
	}
}
dhtmlXTreeObject.prototype.HideChildList=function(item){
	if(item == null) return;
	treeHideChildList = item;
	var i=0;
	for(i=0;i<item.length;i++){
		if(item[i] == '') continue;
		var nodStr = this.getAllSubItems(item[i]);
		var item2 = nodStr.split(',');
		for(j=0;j<item2.length;j++){
			this.showItemCheckbox(item2[j],false);
		}
	}
}
dhtmlXTreeObject.prototype.DisableCheckList=function(item){
	if(item == null) return;
	treeDisableCheckList = item;
	var i=0;
	for(i=0;i<item.length;i++){
		if(item[i] == '') continue;
		this.disableCheckbox(item[i],true);
	}
}
//treeHideItemList
dhtmlXTreeObject.prototype.HideItemList=function(item){
	if(item == null) return;
	treeHideItemList = item;
	var i=0;
	for(i=0;i<item.length;i++){
		if(item[i] == '') continue;
		this.showItemCheckbox(item[i],false);
	}
}
//treeHideItemList
dhtmlXTreeObject.prototype.ShowItemList=function(item){
	if(item == null) return;
	treeShowItemList = item;
	var i=0;
	for(i=0;i<item.length;i++){
		if(item[i] == '') continue;
		this.showItemCheckbox(item[i],true);
	}
}
