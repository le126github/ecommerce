(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            ...
            title: false, //是否包含标题
            pagestat: '从{from}到{to}，共{total}条记录', //显示当前页和总页面的样式
            ...
            autoload: true, //自动加载
            autoloadGetQuery: false, //自动加载时是否取表单的查询条件数据
	        ...
	        sortname: "1",
        	sortorder: "asc",
            rowhandler: false, //是否启用行的扩展事件功能
            rowbinddata: false,
            getQuery:false,//返回查询字符串 shoru添加，用以添加外部表单参数
            extParam: '',
            //Style
            gridClass: "bbit-grid",
            onrowchecked: false
        ...
        var g = {
                ...
                else if (dragtype == 'colMove') //column header drag
                {
                    ...
                    if ($.browser.msie) {
                        this.colCopy.className = "colCopy ie";
										...
										//shoru fix the drag col head font-size bug
                    $(this.colCopy).css({ position: 'absolute', float: 'left', display: 'none', textAlign: obj.align, "font-size":$(obj).css("font-size") });
                            ...
                            $(ths).each(function(j) {
                                ...
                                if (idx == "-1") { //checkbox
                                    ...
                                    if (tdclass != "") {
                                		...
                                		//shoru添加：该列数据通过ref来引用其他列的数据
                       /*         		if(this.ref && parseInt(this.ref) >= 0){
                                			 divInner = row.cell[parseInt(this.ref)] || "&nbsp;";
                                		}
                         */           
                                    if (this.process) {
                                        //divInner = this.process(divInner, trid);
                                        //lijq添加20110613：在行处理方法中增加本行数据
                                        divInner = this.process(divInner, trid, row.cell.join("_FG$SP_"));
                                    }
                                    div.push(divInner);
                                }
                                div.push("</div>");
				 	...
				 	function() {
				 	    ...
				 	    $(ths).each(function(j) {
				 	        ...
				 	        if (idx == "-1") { //checkbox
				 	            ...
				 	            /*
								if(this.ref && parseInt(this.ref) >= 0){
                          			divInner = arrdata[parseInt(this.ref)] || "&nbsp;";
                      			}*/
				 	            if (p.rowbinddata) {
				 	                tddata = arrdata[idx] || arrdata;
				 	            }
				 	            if (this.process) {
				 	                divInner = this.process(divInner, trid);
				 	            }
            ...
            populate: function() {
				...
				//shoru 添加 获取extParam
				//todo:bugfix 会覆盖原有的extParam
		/*		if(p.extParamBuilder){
					p.extParam=p.extParamBuilder;
				}*/
                ...
                $.ajax({
                    ...
                    url: p.url,
                    data: param,
                  //  timeout: 5*1000,//shoru 为ajax请求添加超时，10秒
                    dataType: p.dataType,
                    success: function(data) { if (data != null && data.error != null) { if (p.onError) { p.onError(data); g.hideLoading(); } } else { g.addData(data); } },
                    //错误提示
                    error: function(data) { try { if (p.onError) { p.onError(data); } else { if(msg.description){alert(msg.description);}else{alert('发生错误')}} g.hideLoading(); } catch (e) { } }
                });
            },
            doSearch: function() {
            	//shoru 添加查询扩展参数
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = '<div class="pGroup"><div class="pFirst pButton" title="转到第一页"><span></span></div><div class="pPrev pButton" title="转到上一页"><span></span></div> </div> <div class="pGroup"><span class="pcontrol">当前第<input type="text" size="1" value="1" />页/共<span>1</span>页</span></div><div class="btngo"><span/></div><div class="pGroup"> <div class="pNext pButton" title="转到下一页"><span></span></div><div class="pLast pButton" title="转到最后一页"><span></span></div></div><div class="btnseparator"></div><div class="pGroup"> <div class="pReload pButton" title="刷新"><span></span></div> </div> <div class="btnseparator"></div><div class="pGroup"><span class="pPageStat"></span></div><div class="btnseparator"></div>';
            ...
            //添加了go按钮事件
         // $('.btngo').click=g.changePage('input');
            ...
            if (p.searchitems) {
                ...
                if (p.qtype == '') p.qtype = sitems[0].name;
								//shoru添加了搜索按钮
                $(g.sDiv).append("<div class='sDiv2'>快速检索：<input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>" + sopt + "</select> <input type='button' name='qsearchbtn' value='查询'/> <input type='button' name='qclearbtn' value='清空' /></div>");

                $('input[name=q],select[name=qtype]', g.sDiv).keydown(function(e) { if (e.keyCode == 13) g.doSearch() });
                $('input[name=qclearbtn]', g.sDiv).click(function() { $('input[name=q]', g.sDiv).val(''); p.query = ''; g.doSearch(); });
                //shoru added 添加了搜索按钮事件
                $('input[name=qsearchbtn]', g.sDiv).click(function() {g.doSearch();});
                $(g.bDiv).after(g.sDiv);
	 ...
	 //  		g.mDiv.innerHTML='<h4 class="ftitle"><span>'+p.title+'</span></h4>';
        ...
        // load data
        if (p.url && p.autoload) {
        	if(p.autoloadGetQuery){
        		//alert("p.autoloadGetQuery:"+p.autoloadGetQuery);
        		g.doSearch();
        	} else {
        		g.populate();
        	}
